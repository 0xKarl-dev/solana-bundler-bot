...
export class BaseRay {
  ...
  async addLiquidity(
    ...
    if (typeof baseAmount == "number" && typeof quoteAmount == "number") {
      baseAmountIn = calcNonDecimalValue(
        baseAmount,
        baseMintDecimals
      ).toString();
      quoteAmountIn = calcNonDecimalValue(
        quoteAmount,
        quoteMintDecimals
      ).toString();
    } else {
  ...
  async removeLiquidity(
  ...
  ): Promise<Result<{ ixs: web3.TransactionInstruction[] }, string>> {
    ...
    const lpTokenAccountInfo =
      await this.connection.getAccountInfo(lpTokenAccount);
    if (!lpTokenAccountInfo) return { Err: "No lp token found" };
    const totalLp = Number(
      AccountLayout.decode(lpTokenAccountInfo.data).amount.toString()
    );
